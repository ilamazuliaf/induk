...
class Login():
	...
	def login(self):
		data = requests.get(self.url+'auth/login', auth=('maryam','maryam'), headers=self.headers)
...
class Pedatren(Login):
	...
	def updateInduk(self, uuid, induk, id_lembaga, id_pendidikan, tanggal_masuk):#, rombel):
		payloads = {
			"nomor_induk": "{}".format(induk),
			"id_lembaga": "{}".format(id_lembaga),
